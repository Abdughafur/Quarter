...
window.app = {
  ...
  add(val) {
    if (this.grades.length >= 50) {
      this.showToast("⚠️ Максимум 50 балл! Дигар наметавонед илова кунед.");
  ...
  updateUI() {
    ...
    avgEl.classList.remove("avg-gradient", "avg-shiny");
    avgEl.classList.add(final >= 10 ? "avg-shiny" : "avg-gradient");
  ...
  runAI(avg, avgReg, avgEx, cReg, cEx) {
    ...
    if (isPerfect) {
      positiveHTML = `🎉 Хонанда чоряки <b>10</b> (аъло) дорад!<br>Ин хеле олӣ аст!`;
    } else {
      let target = Math.floor(avg) + 0.5;
      if (avg >= target) target += 0.5;
      let nextGrade = Math.min(10, Math.ceil(target));
      let neededRegAvg =
        cEx > 0 ? target * 2 - avgEx : cReg > 0 ? target : target;
      let n10 = 0,
        n9 = 0,
        n8 = 0;
      ...
      positiveHTML = `Барои ба <b>${nextGrade}</b> расонидан:<br>• 10: <b>${n10}</b> маротиба<br>• 9: <b>${n9}</b> маротиба<br>• 8: <b>${n8}</b> маротиба`;
    }
    aiText.innerHTML = positiveHTML;

    let dropPoint = Math.floor(avg) - 0.5;
    let neededRegDrop =
      cEx > 0 ? dropPoint * 2 - avgEx : cReg > 0 ? dropPoint : dropPoint;
    let nBad = 0,
      n3 = 0,
      n4 = 0;
    ...
    riskText.innerHTML =
      nBad > 0 && nBad < 20
        ? `Агар паст шавад (ба <b>${Math.floor(avg) - 1}</b>):<br>• 2: <b>${nBad}</b> маротиба<br>• 3: <b>${n3}</b> маротиба<br>• 4: <b>${n4}</b> маротиба`
        : "Баллҳо дар ҳолати начандон бад қарор дорад.";
  },
